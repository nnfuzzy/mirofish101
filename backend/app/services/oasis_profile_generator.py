...
class OasisProfileGenerator:
    ...
    def __init__(
        ...
        # Resolve model: provider-prefixed arg > LITELLM_MODEL env > legacy.
        ...
        # If LITELLM_MODEL is in play, route through LiteLLM and let it read
        # provider keys (GEMINI_API_KEY, ANTHROPIC_API_KEY, …) from env.
        # Otherwise fall back to the legacy OpenAI-compatible client.
        use_litellm = bool(Config.LITELLM_MODEL) or (model_name and "/" in model_name)
        if use_litellm:
            self.api_key = api_key  # may be None — LiteLLM reads from env
            self.base_url = base_url  # may be None
