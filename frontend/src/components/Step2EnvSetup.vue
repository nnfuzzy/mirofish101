            ...
            <div class="model-cards">
              ...
              >
                <input
                  ...
                  class="model-card-radio"
                />
                <div class="model-card-name">{{ m.display_name }}</div>
                <div class="model-card-tagline">{{ m.tagline }}</div>
                <div class="model-card-cost">{{ m.cost_tier }}</div>
              </label>
            </div>
          </div>
...
const handleStartSimulation = () => {
  ...
  if (selectedModel.value) {
    params.selectedModel = selectedModel.value
  }
...
onMounted(async () => {
  ...
  try {
    ...
    selectedModel.value = data.default_model_id
  } catch (e) {
    console.error('Failed to load models, falling back to env default', e)
  }
...
.model-card {
  ...
  border-radius: 6px; cursor: pointer; transition: border-color 120ms, background 120ms;
}
.model-card:hover { border-color: #888; }
.model-card.active { border-color: #2196f3; background: #f0f7ff; }
.model-card-radio { display: none; }
.model-card-name { font-weight: 600; }
.model-card-tagline { color: #666; font-size: 12px; margin: 4px 0; }
.model-card-cost { color: #2196f3; font-weight: 600; font-size: 14px; }
</style>
